...
from client.download import DownloadResult, _iqm as dl_iqm
from client.upload import UploadResult, _iqm as ul_iqm
from client.stats import ConnectionStats, LatencyStats
...
class TestIqmFunction(unittest.TestCase):
    """Test the _iqm helper used by both download and upload."""

    def test_empty(self):
        self.assertEqual(dl_iqm([]), 0.0)
        self.assertEqual(ul_iqm([]), 0.0)

    def test_few(self):
        self.assertAlmostEqual(dl_iqm([10.0, 20.0, 30.0]), 20.0)

    def test_outlier_resistant(self):
        # Extreme outliers should be trimmed
        samples = [50, 50, 50, 50, 50, 50, 500, 1]
        result = dl_iqm(samples)
